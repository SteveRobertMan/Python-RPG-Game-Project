...
def get_enemy_database():
    ...
    sp_kiryoku.max_hp = 80
    k_sp_kiryoku = Kata("Kiryoku Sparring", "Kiryoku Student", 1, 1, [1.0, 1.0, 1.0, 1.5, 1.4, 1.0, 1.2])
    ...
    c_kiryoku.max_hp = 90
    k_c_kiryoku = Kata("Council Combat", "Council Combatant", 1, 2, [1.3, 1.0, 1.0, 1.1, 1.3, 1.4, 1.4])
    ...
    inf_kasa.max_hp = 64
    k_inf_kasa = Kata("Fake Karateka", "Kasakura Infiltrator", 1, 0, [1.5, 1.2, 1.1, 1.0, 1.4, 1.4, 1.0])
    ...
    inf_lead.max_hp = 80
    k_inf_lead = Kata("Fake Leader", "Infiltrator Leader", 1, 1, [1.2, 1.2, 1.2, 1.6, 1.3, 1.3, 1.0])
    ...
    hi1 = Skill("Following Orders", 1, EL_STORGE, 4, "[On Use] This unit takes -40% damage next turn\n[On Hit] If this unit has Haste, gain 1 Bind next turn. Otherwise, gain 1 Haste next turn", effect_type="HISAYUKI_SPECIAL_1")
    hi2 = Skill("Pick Up Speed", 2, EL_LUDUS, 8, "[Combat Start] If this unit has Bind, take +50% damage for the turn\n[On Hit] If this unit has no Haste, gain 3 Haste next turn, otherwise, gain 1 Haste next turn", effect_type="HISAYUKI_SPECIAL_2")
    hi3 = Skill("Human Battering Ram", 3, EL_EROS, 11, "[On Hit] If this unit has Haste, deal +10% damage for every stack of Haste on self (Max +50%), then remove all Haste on self", effect_type="HISAYUKI_SPECIAL_3")
    ...
    inf_council.max_hp = 76
    k_inf_council = Kata("Fake Council", "Fake Council", 1, 1, [1.1, 1.4, 1.5, 1.0, 1.2, 1.5, 1.3])
    ...
    inf_disc.max_hp = 70
    k_inf_disc = Kata("Fake Discipline", "Fake Disciplinary", 1, 1, [1.2, 1.2, 1.0, 1.3, 1.4, 1.4, 1.0])
    ...
    ea1 = Skill("Pressuring", 1, EL_PHILAUTIA, 6, "[On Hit] Target will take +5 Final Damage from the next attack\n[On Hit] Target deals -50% damage next turn", effect_type="EAGLE_SPECIAL_1")
    ea2 = Skill("Under Control", 2, EL_PHILAUTIA, 10, "[On Use] Gain 3 Poise Potency\n[On Use] Gain 3 Poise Count\n[On Hit] Inflict 2 Rupture Potency\n[On Hit] Inflict 5 Rupture Count", effect_type="EAGLE_SPECIAL_2")
    ea3 = Skill("Tactical Retreat", 3, EL_STORGE, 20, "[On Use] Deal 0 damage, heal this unit’s ally with the lowest HP by supposed final damage amount, then heal the rest of this unit’s allies by half the healed amount (can include self)\n[On Use] All of this unit’s allies gain 3 Haste next turn", effect_type="EAGLE_SPECIAL_3")
    ...
    rip_hench.max_hp = 105
    k_rip_hench = Kata("Gang Rapier", "Riposte Henchman", 1, 4, [1.2, 1.1, 1.1, 1.1, 0.9, 0.9, 1.1])
    ...
    rip_lead.max_hp = 130
    k_rip_lead = Kata("Veteran Rapier", "Riposte Leader", 1, 6, [1.1, 1.0, 1.0, 1.2, 0.9, 0.9, 1.0])
